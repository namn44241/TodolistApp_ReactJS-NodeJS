...
const sendTaskNotification = async (email, tasks) => {
  console.log(`\nĐang chuẩn bị gửi mail cho ${email}`);
  console.log('- Tạo nội dung mail...');

  // Tạo HTML cho danh sách tasks
  const tasksHtml = tasks.map(task => `
    <div style="margin-bottom: 20px; padding: 15px; border: 1px solid #ddd; border-radius: 5px;">
      <h3 style="margin: 0 0 10px 0; color: #333;">${task.title}</h3>
      <p style="margin: 5px 0;"><strong>Hạn:</strong> ${new Date(task.due_date).toLocaleDateString('vi-VN')}</p>
      ${task.description ? `<p style="margin: 5px 0;"><strong>Mô tả:</strong> ${task.description}</p>` : ''}
    </div>
  `).join('');
  ...
  const mailOptions = {
    ...
    to: email,
    subject: `Bạn có ${tasks.length} công việc sắp đến hạn`,
    html: `
      <div style="font-family: Arial, sans-serif; padding: 20px;">
        <h2 style="color: #2c3e50;">Công việc sắp đến hạn</h2>
        <p>Xin chào,</p>
        <p>Bạn có ${tasks.length} công việc sắp đến hạn trong 3 ngày tới:</p>
